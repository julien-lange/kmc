                ...
                for y in range(miny,maxy):
                    for p in range(minp,maxp):
                        ...
                        for it in range(1,maxiterations):
                            ...
                            kmccmd = subprocess.Popen(["./KMC",tmpfile,"1",str(y),"--fsm","--debug"], stdout=subprocess.PIPE)
                            try:
...
### THESE ARE THE PARAMETERS USED TO GENERATED FIG 6
### (kept here for easy comparison)
...
# runOverRange(1, 50, 2, 3, 1, 2, False) # this times out before 50



### SUGGESTED PARAMETERS FOR QUICK EVALUATION

# Measure growing k with 10 peers
runOverRange(1, 2, 2, 50, 5, 6, False) # Should be about 1h on the VM

# Measure growing number of peers with k=10
runOverRange(1, 2, 10, 11, 1, 17, False) # Should be about 1h on the VM

# Measure growing number of branchings with k=2 and peers=2
runOverRange(1, 9, 2, 3, 1, 2, False) # Should be about 20min on the VM
